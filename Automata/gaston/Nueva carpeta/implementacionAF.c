...
void AceptaCadena(T_AF A){
	...
	Tcadena leido,cadCom,auxcadCom, probar;
	...
	while(cara!='\0'){
		...
		while(auxAnt!=NULL){//auxAnt CONJUNTO DE ESTADOS
			...
			while(auxTransiciones!=NULL ){//auxTransiciones CONJUNTO DE TERNAS
				...
				strcpy(cadCom,auxcadCom);
				strcpy(probar,auxcadCom);
				printf("elemento 1: %s, elemento 2: %s", entreComa(probar).valor, carga_caracter(cara).valor);
				//system("pause");
				...
				retornaValor(elem,auxcadCom);
				strcpy(cadCom,auxcadCom);
				printf("\nmuestra b1: %d, muestra b2: %d", b1, b2);
				system("pause");
				if(b1==0 && b2==0){
				//if(compara_ele(antesComa(cadCom),elemAnt)==0 && compara_ele(entreComa(cadCom),carga_caracter(cara))==0){
					...
					muestra_lista(nuevo);
					system("pause");
					printf("salio primer if");
				}
				auxTransiciones=auxTransiciones->sig;
			}
			printf("salio tercer while");
			auxAnt=auxAnt->sig;
			printf("\nmuestra aux_ant");
			muestra_lista(auxAnt);
			system("pause");
		}
	...
	muestra_lista(interseccion(ant,A.F));
	system("pause");
